...
## 2026-07-08 — REF-24 「홈 간트·GNB 5개·업무 상세 팝업」 구현 (branch: feat/ref-24-home-gantt-gnb-popup)

근거: `docs/20260708_기획변경요구_홈간트_GNB_업무상세팝업_v0.1.md`(결정 D8~D11) + 정의서 v1.1 3종 + 화면계획 PNG 2장(SCR-CM-03/SCR-WF-02).

- **DEV-5 mock**: `codes.js` HOME_CATEGORY 7종(EDU/ETC 신설)·GANTT_SCOPE(주간/월간/연간)·PROCESS_VIEW(일/주/월) · TASK 확장 costBudget/durationDays/linkedFormIds(파생 시드) · 목업데이터 v1.1 재추출.
- **DEV-2 GNB(D9·D10)**: 5메뉴(규정/업무/DATA연계/AI/모니터링) · `/bi/*`→`/monitoring/*` 개칭(참조 전수 갱신) · AI-03 QnA `/ai/qna` 독립(규정 LNB에서 제외) · 조직도 GNB 제외 — 홈 조직 트리 '조직도 →' 진입.
- **DEV-4**: `SCR-DL-01` DATA연계 placeholder(`/data-link`) — 상세 기획 [확인필요].
- **DEV-3 WF-02(D11)**: Drawer→중앙 팝업(`PopupShell` 신설·WF02-08/10) · 헤더=개요+핵심 BI 스파크라인+소요시간·원가(WF02-15) · 탭 6→5(평가·댓글 제외 [확인필요], WF02-03) · `ProcessMap` 신설(일/주/월 토글 WF02-16·타부서 연계 저오퍼시티·회의/규정 연결 노드 WF02-17·스코프 타임라인) · 모니터링 탭=B1/B4/B6/B7 재사용 taskId 컨텍스트(WF02-18). REF-23 결재선 타임라인·✦AI 배너·상신 전이·ReportDraft 전부 유지.
- **DEV-1 CM-03(D8)**: 컬럼 드릴다운 홈 폐지 → 조직 트리 LNB(CM03-25)+카테고리 7종(CM03-17)+담당자별 커스텀 간트(업무명 바·정기 뱃지·상태색·연관 점선 SVG·타부서 저오퍼시티 CM03-27·결재 칩 CM03-22 이관·주간/월간/연간 CM03-18)+우측 목표 BI 패널(CM03-28)+DATA 패널(규정/양식/결재 현황 CM03-26·`ApprovalMini` 공용화 CM03-22~24) · 바 클릭=선택·더블클릭=WF-02 팝업(CM03-19 [확인필요: 구분]) · EmptyState 문구 변경(CM03-21). `BiDrawer` 삭제(폐지).
- **검증**: `npm run build` 성공 · Playwright 스모크 13항목 통과(GNB 5메뉴·EmptyState·간트 12바·기간 전환 변동·DATA 패널·팝업 KPI·모니터링 탭·프로세스맵 일 뷰·DL-01·/monitoring — 콘솔 에러 0).
- **[확인필요] 잔존(REF-24 §7 10건)**: 평가·댓글 거취 · 원가/소요시간 기준 · 클릭/더블클릭 구분(더블클릭 채택) · 결재 칩 위치(간트 바 채택·트리 롤업 보류) · 하위 부서 포함 범위(하위 전체 채택) · 기간별 시드(기준일 2026-03-02 채택) · DL-01 상세 · 양식 연결 규칙 · 연결 노드 이동 방식 · 상태 필터 유지 여부.
- **후속**: REF-12 specs(SCR-CM-03/WF-02 v1.1 정합)·DEF-04 라우팅 맵 갱신은 기획 [확인필요] 해소 후 진행.
